...
def search_bank(query, cls, subject, stream, limit=10):
    q = norm(query)
    words = [x for x in re.findall(r"\w+", q) if len(x) > 2]
    ...
    # Exact question match always wins.
    exact = [r for score, r in scored if norm(r.get("question", "")) == q]
    if exact:
        return exact[:1]

    # If there is no exact match, show only the single best match.
